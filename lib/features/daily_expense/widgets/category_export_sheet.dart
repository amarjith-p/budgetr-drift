...
class CategoryExportSheet extends StatelessWidget {
  ...
  Future<void> _generateAndSaveCSV(BuildContext context) async {
    try {
      ...
      // [FIXED] Removed UTF-8 BOM ([0xEF, 0xBB, 0xBF]) to prevent symbols in header
      final List<int> bytes = utf8.encode(csvData);

      final safeName = filterAccountName.replaceAll(RegExp(r'[^\w\s]+'), '');
      final fileName = "BudgetR_${safeName}_Report.csv";
  ...
  Future<void> _generateAndSavePDF(BuildContext context) async {
    try {
        ...
        pw.MultiPage(
          ...
          build: (pw.Context context) => [
            ...
            if (incomeItems.isNotEmpty) ...[
              ...
              _buildPdfTable(context, incomeItems, totalIncome, currency),
              pw.SizedBox(height: 20),
            ],
            if (expenseItems.isNotEmpty) ...[
              ...
              pw.SizedBox(height: 10),
              _buildPdfTable(context, expenseItems, totalExpense, currency),
            ],
            pw.SizedBox(height: 20),
      ...
      final safeName = filterAccountName.replaceAll(RegExp(r'[^\w\s]+'), '');
      final fileName = "BudgetR_${safeName}_Report.pdf";
  ...
  pw.Widget _buildPdfTable(pw.Context context,
